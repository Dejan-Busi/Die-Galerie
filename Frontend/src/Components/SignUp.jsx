...
function SignUp() {
  ...
  const [arrayErrorMessages, setArrayErrorMessages] = useState([]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (error.response.status == 400) {
        console.log("Email registriert oder Passwörter stimmen nicht überein:", error);
        const errorMessage = error.response.data;
        setSingleErrorMessage(errorMessage);
        setArrayErrorMessages([]);

      } if (error.response.status == 500) {
        console.log("Passwortkriterien nicht erfüllt:", error);
        const arrayErrorMessages = error.response.data;
        setArrayErrorMessages(arrayErrorMessages);
        setSingleErrorMessage("");

      } else if (error.response.data.errors) {
        console.log("Bitte fülle alle Felder aus:", error);
  ...
  return (
    <div>
      <div id="signUpPage">
              ...
              <div id="flexbox_button">
                <button type="submit" className="button">
                  {loading ? (
                    ...
                    <p style={{ margin: 0 }}> Registrieren</p>
                  )}
                </button>
              </div>
              <br /> <br />
        ...
        <br /> <br />
        <p style={{ color: "red", fontSize: "1.5rem" }}>{singleErrorMessage}</p>
        {arrayErrorMessages.map((elem) => (
          <div key={elem.code}>
        ...
        <section id="startSeiteFlexbox">
          ...
          <div id="behind_the_Scenes">
            ...
            >
              <p
                className="navLink"
                style={{ fontSize: "2rem", color: "lightcoral" }}
              >
                <img
                  id="actionClapperSignup"
                  src={actionClapper}
                  alt="Action Clapper"
                />
                Behind the Scenes
          ...
          <div id="vorgeschmack_App">
            <h2 style={{ color: "black", fontSize: "1.7rem" }}>
              Ein Vorgeschmack der App:
            </h2>
            <NavLink
              to="/lobby"
              style={{ margin: "auto" }}
